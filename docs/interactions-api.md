...
| **`response_format`** | `dict` | e.g. `{"type": "image"}`. Supports `"aspect_ratio"`: `"1:1"`, `"16:9"`, `"9:16"`, `"4:3"`, `"3:4"`. |
...
# 4. Save the result. google-genai 2.x returns `steps`; the image is exposed as
# `output_image`, whose `data` is base64 text or raw bytes.
image = local_edit.output_image
data = image.data
...
> [!IMPORTANT]
> Use `google-genai` 2.x. The API removed the legacy `outputs` response schema on 2026-06-08 and rejects 1.x SDKs with HTTP 400.
